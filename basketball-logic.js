...
export function createBasketball(player, pursuer) {
  const x = player.x + player.width + 5;
  const y = player.y + player.height - BALL_SIZE - 3;
  const aim = pursuerTarget(pursuer);
  const targetX = aim.x + aim.width / 2 + Math.max(0, pursuer.velocity ?? 0) * 0.28;
...
export function updateBasketball(ball, previousPursuer, pursuer, elapsedMs, worldEnd, random = Math.random) {
  ...
  if (sweptOverlaps(previousBall, nextBall, previousTarget, currentTarget)) {
